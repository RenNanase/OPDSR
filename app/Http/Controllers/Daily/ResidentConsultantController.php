...
use App\Models\ResidentConsultant; // The model for consultant names and suites
use App\Models\ResidentConsultantLog; // The model for daily log entries
use App\Models\ResidentConsultantLogForeigner; // The model for foreigner details
use App\Models\ResidentConsultantPatient; // The model for patient statistics
...
class ResidentConsultantController extends Controller
{
    /**
     * Display the list of Resident Consultant daily logs.
     */
    public function index()
    {
        ...
        $loggedEntries = ResidentConsultantLog::with(['patients', 'foreignerPatients'])
            ->whereDate('date', $date)
            ->orderBy('created_at', 'desc')
                                                ->get();

        // Get all consultants
        $allConsultants = ResidentConsultant::all();
        ...
        foreach ($loggedEntries as $log) {
            ...
            if ($patient) {
                ...
                $dailyStats['new_male_count'] += $patient->new_male_count ?? 0;
                $dailyStats['new_female_count'] += $patient->new_female_count ?? 0;
            }
        ...
        $dailyStats['total_patients'] = $dailyStats['male_count'] + $dailyStats['female_count'] +
                                      $dailyStats['new_male_count'] + $dailyStats['new_female_count'] +
                                      $dailyStats['foreigner_male_count'] + $dailyStats['foreigner_female_count'];

        return view('daily.resident-consultant.index', compact('loggedEntries', 'missingConsultants', 'dailyStats', 'selectedDate'));
    }

    /**
     * Show the form for creating a new Resident Consultant log entry.
     */
    public function create()
    {
        $consultants = ResidentConsultant::all();
        $selectedDate = request('date', Carbon::today()->format('Y-m-d'));

        // Get existing logs for the selected date
        $existingLogs = ResidentConsultantLog::whereDate('date', $selectedDate)
        ...
        return view('daily.resident-consultant.create', compact('consultantsWithStatus', 'selectedDate'));
    }

    /**
     * Display the specified Resident Consultant log entry.
     */
    public function show(ResidentConsultantLog $entry)
    {
        $entry->load(['foreignerPatients', 'patients']);
        return view('daily.resident-consultant.show', compact('entry'));
    }

    /**
     * Store a new Resident Consultant daily log entry.
     */
    public function store(Request $request)
    {
        // Validation rules
        $rules = [
            'consultant_name'      => ['required', 'string', 'exists:resident_consultants,name'],
            ...
            'new_male_count'       => ['required', 'integer', 'min:0'],
            'new_female_count'     => ['required', 'integer', 'min:0'],
        ...
        try {
            ...
            $existingLog = ResidentConsultantLog::where('date', $validatedData['date'])
                                                ->where('consultant_name', $validatedData['consultant_name'])
                                                ->first();

            if ($existingLog) {
                return redirect()->route('daily.resident-consultant.index')
            ...
            $logEntry = new ResidentConsultantLog();
            ...
            $patientStats = new ResidentConsultantPatient();
            $patientStats->resident_consultant_log_id = $logEntry->id;
            $patientStats->chinese_count = $validatedData['chinese_count'];
            $patientStats->malay_count = $validatedData['malay_count'];
            $patientStats->india_count = $validatedData['india_count'];
            $patientStats->kdms_count = $validatedData['kdms_count'];
            $patientStats->others_count = $validatedData['others_count'];
            $patientStats->male_count = $validatedData['male_count'];
            $patientStats->female_count = $validatedData['female_count'];
            $patientStats->new_male_count = $validatedData['new_male_count'];
            $patientStats->new_female_count = $validatedData['new_female_count'];
            $patientStats->save();
            ...
            if (isset($validatedData['foreigner_countries']) && is_array($validatedData['foreigner_countries'])) {
                ...
                $maleCount = $validatedData['foreigner_male_count'] ?? 0;
                $femaleCount = $validatedData['foreigner_female_count'] ?? 0;
            ...
            return redirect()->route('daily.resident-consultant.index')
                            ->with('status', 'Resident Consultant log saved successfully!');

        } catch (\Exception $e) {
            Log::error('Resident Consultant log failed: ' . $e->getMessage());
            return redirect()->route('daily.resident-consultant.index')
    ...
    /**
     * Display the monthly records for Resident Consultants.
     */
    public function monthly(Request $request)
    {
        ...
        $logs = ResidentConsultantLog::with(['patients', 'foreignerPatients'])
            ...
            for ($day = 1; $day <= $daysInMonth; $day++) {
                ...
                if ($log) {
                    ...
                    if ($patient) {
                        ...
                        $monthlyStats['new_male_count'] += $patient->new_male_count ?? 0;
                        $monthlyStats['new_female_count'] += $patient->new_female_count ?? 0;
                        $monthlyStats['new_patients_count'] += ($patient->new_male_count ?? 0) + ($patient->new_female_count ?? 0);
                    }
        ...
        return view('daily.resident-consultant.monthly', compact(
            'consultants',
            'monthlyData',
            'monthlyStats',
            'selectedMonth',
    ...
    public function edit(ResidentConsultantLog $entry)
    {
        try {
            ...
            return view('daily.resident-consultant.edit', compact('entry'));
        } catch (\Exception $e) {
            \Log::error('Error in ResidentConsultantController@edit: ' . $e->getMessage());
            return redirect()->route('daily.resident-consultant.index')
                ->with('error', 'An error occurred while loading the edit form. Please try again.');
        }
    }

    public function update(Request $request, ResidentConsultantLog $entry)
    {
        ...
        try {
            DB::transaction(function () use ($validated, $entry) {
                ...
                $entry->patients()->update([
                    ...
                    'new_male_count' => $validated['new_male_count'],
                    'new_female_count' => $validated['new_female_count'],
                ]);
            ...
            return redirect()->route('daily.resident-consultant.index')
                ->with('status', 'Resident consultant log updated successfully.');
        } catch (\Exception $e) {
            Log::error('Error updating resident consultant log: ' . $e->getMessage());
            return redirect()->route('daily.resident-consultant.index')
    ...
    public function export(Request $request)
    {
        ...
        $records = ResidentConsultantLog::with(['patients', 'foreignerPatients'])
        ...
        // Set headers
        $sheet->setCellValue('A1', 'Resident Consultant Monthly Report - ' . date('F Y', mktime(0, 0, 0, $month, 1, $year)));
        ...
        $row++;
        $sheet->setCellValue('A' . $row, 'Male');
        $sheet->setCellValue('B' . $row, $monthlyStats['new_male_count']);
        $row++;
        $sheet->setCellValue('A' . $row, 'Female');
        $sheet->setCellValue('B' . $row, $monthlyStats['new_female_count']);
        $row++;
        ...
        $filename = 'Resident_Consultant_Report_' . date('F_Y', mktime(0, 0, 0, $month, 1, $year)) . '.xlsx';
