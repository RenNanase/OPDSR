...
use App\Models\MedicalProcedureOW;
use Carbon\Carbon;
...
class OldWingController extends Controller
{
    public function index(Request $request)
    {
        $date = $request->input('date', Carbon::today()->format('Y-m-d'));
        $record = MedicalProcedureOW::where('date', $date)->first();

        return view('annual.old-wing', compact('record', 'date'));
    }
    public function daily(Request $request)
    {
        $date = $request->input('date', Carbon::today()->format('Y-m-d'));
        $procedures = MedicalProcedureOW::where('date', $date)->first();

        return view('annual.old-wing-daily', compact('procedures', 'date'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $procedures = new MedicalProcedureOW();
            ...
            return redirect()->route('annual.old-wing.index', ['date' => $validated['date']])
    ...
    public function monthly(Request $request)
    {
        ...
        // Get daily records with latest data
        $dailyRecords = MedicalProcedureOW::whereBetween('date', [$startDate, $endDate])
            ->orderBy('date')
            ->get()
        ...
        // Calculate totals with latest data
        $totals = MedicalProcedureOW::whereBetween('date', [$startDate, $endDate])
            ->select([
                DB::raw('COALESCE(SUM(injection_vaccine), 0) as total_injection_vaccine'),
                DB::raw('COALESCE(SUM(iv_medication), 0) as total_iv_medication'),
                DB::raw('COALESCE(SUM(urea_blood_test), 0) as total_urea_blood_test'),
                DB::raw('COALESCE(SUM(venepuncture), 0) as total_venepuncture'),
                DB::raw('COALESCE(SUM(iv_cannulation), 0) as total_iv_cannulation'),
                DB::raw('COALESCE(SUM(swab_cs_nose_oral), 0) as total_swab_cs_nose_oral'),
                DB::raw('COALESCE(SUM(dressing), 0) as total_dressing'),
                DB::raw('COALESCE(SUM(ecg_12_led), 0) as total_ecg_12_led'),
                DB::raw('COALESCE(SUM(urinary_catheterization), 0) as total_urinary_catheterization'),
                DB::raw('COALESCE(SUM(ng_tube_insertion), 0) as total_ng_tube_insertion'),
                DB::raw('COALESCE(SUM(nebulization), 0) as total_nebulization'),
            ])
            ->first();

        // Log the totals for debugging
        Log::info('Monthly totals for ' . $selectedMonth . '/' . $selectedYear, [
            'totals' => $totals,
            'start_date' => $startDate->format('Y-m-d'),
            'end_date' => $endDate->format('Y-m-d'),
            'total_procedures' => $totals->total_injection_vaccine +
        ...
        return view('annual.old-wing-monthly', compact('daysInMonthArray', 'daysInMonth', 'selectedMonth', 'selectedYear', 'totals'));
    }
    ...
    public function export(Request $request)
    {
        try {
            ...
            // Set title
            $sheet->setCellValue('A1', 'Old Wing Monthly Medical Procedures - ' . date('F Y', mktime(0, 0, 0, $selectedMonth, 1, $selectedYear)));
            ...
            $filename = 'old_wing_monthly_procedures_' . date('F_Y', mktime(0, 0, 0, $selectedMonth, 1, $selectedYear)) . '.xlsx';
    ...
    {
        $procedures = MedicalProcedureOW::where('date', $date)->firstOrFail();
        return view('annual.old-wing-edit', compact('procedures', 'date'));
    }
    ...
    public function update(Request $request, $date)
    {
        ...
        try {
            ...
            return redirect()->route('annual.old-wing.daily', ['date' => $date])
