@props(['showingSidebar', 'isMinimized']) {{-- Accept props from app.blade.php --}}

<aside class="fixed h-full shadow-lg z-40 flex flex-col transition-all duration-300 ease-in-out
       bg-opd-primary text-white"
       :class="{
           'w-64': !isMinimized || showingSidebar,  // Full width when not minimized (desktop) or showing (mobile overlay)
           'w-20': isMinimized && !showingSidebar, // Minimized width when collapsed (desktop)
           'translate-x-0': showingSidebar || window.innerWidth >= 768, // Always visible on desktop, or when showing on mobile
           '-translate-x-full': !showingSidebar && window.innerWidth < 768 // Hidden on mobile unless toggled
       }">
    <div class="flex items-center justify-between h-16 bg-opd-primary text-black text-2xl font-bold border-b border-opd-primary-dark shadow-md px-4">
        <span x-show="!isMinimized" class="whitespace-nowrap">{{ config('app.name', 'OPDSR') }}</span>
        <span x-show="isMinimized" class="whitespace-nowrap text-lg">{{ config('app.name', 'OPDSR')[0] ?? 'O' }}</span> {{-- Show first letter when collapsed --}}

        {{-- Toggle Button for Minimizing (Desktop Only) --}}
        <button @click="isMinimized = !isMinimized" class="hidden md:block text-black hover:text-black
        ...
        @if(auth()->user()->isStaff())
            ...
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open"
                        class="flex items-center w-full px-4 py-2 text-gray-600
                        hover:bg-opd-secondary hover:text-black rounded-md transition duration-150 ease-in-out focus:outline-none">
            ...
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open"
                        class="flex items-center w-full px-4 py-2 text-gray-600 hover:bg-opd-secondary hover:text-black rounded-md transition duration-150 ease-in-out focus:outline-none">
            ...
            {{-- Original Staff Links --}}

            {{-- <x-sidebar-link :href="route('consultant')" :active="request()->routeIs('consultant')">
                <svg class="w-5 h-5 flex-shrink-0" :class="{'mr-2': !isMinimized}" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"></path></svg>
                <span x-show="!isMinimized">{{ __('Consultant') }}</span>
            </x-sidebar-link>
