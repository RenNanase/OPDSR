<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('New Wing Monthly Medical Procedures') }}
        </h2>
    </x-slot>
                ...
                <div class="p-6 text-gray-900">
                    ...
                    <div class="mb-6">
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                            <form action="{{ route('annual.new-wing.monthly') }}" method="GET" class="flex items-center space-x-4">
                                <div>
                                ...
                                <div class="mt-6">
                                    <a href="{{ route('annual.new-wing.export', ['month' => $selectedMonth, 'year' => $selectedYear]) }}" class="inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-500">
                                        {{ __('Export to Excel') }}
                                    </a>
                                </div>
                            </form>
                            <div class="mt-6">
                                <a href="{{route('annual.new-wing.index') }}"
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr>
                                    ...
                                    @for($day = 1; $day <= $daysInMonth; $day++)
                                        @php
                                            $record = $daysInMonthArray[$selectedYear . '-' . str_pad($selectedMonth, 2, '0', STR_PAD_LEFT) . '-' . str_pad($day, 2, '0', STR_PAD_LEFT)] ?? null;
                                        @endphp
                                ...
                                <dl>
                                    <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                        ...
                                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2 font-semibold">
                                            {{ $totals->total_injection_vaccine + $totals->total_iv_medication + $totals->total_urea_blood_test + $totals->total_venepuncture + $totals->total_iv_cannulation + $totals->total_swab_cs_nose_oral + $totals->total_dressing + $totals->total_ecg_12_led + $totals->total_urinary_catheterization + $totals->total_ng_tube_insertion + $totals->total_nebulization }}
                                        </dd>
                                    </div>
