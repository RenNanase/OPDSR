<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit New Wing Medical Procedures - ') }} {{ \Carbon\Carbon::parse($date)->format('d M Y') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('annual.new-wing.update', ['date' => $date]) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="flex items-center justify-end mt-6 space-x-4">
                            <a href="{{ route('annual.new-wing.daily', ['date' => $date]) }}"
